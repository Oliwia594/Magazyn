...
if choice == "📈 Analiza i Statystyki":
    ...
    if not df_p.empty and not df_k.empty:
        ...
        # POPRAWIONA LINIA (nawias jest teraz zamknięty)
                ...
                if st.form_submit_button("🚀 Dodaj Produkt"):
                    if nazwa:
                        supabase.table("Produkt").insert({
                            "nazwa": nazwa, "liczba": int(ilosc), 
                            "cena": float(cena), "kategoria": int(opcje_kategorii[kat_nazwa])
                        }).execute()
...
# --- WIDOK 3: PRZEGLĄD I ZARZĄDZANIE ---
elif choice == "🔍 Przegląd i Stan":
    st.title("🔍 Zarządzanie Stanem")
    df_p = get_products()
        ...
        with col1:
            ...
            with st.form("sub_form"):
                ...
                if st.form_submit_button("⬇️ Potwierdź wydanie"):
                    wybrany = df_p[df_p['nazwa'] == prod_name]
                    aktualny_stan = int(wybrany['liczba'].values[0])
                    prod_id = int(wybrany['id'].values[0])
                    
                    nowy_stan = aktualny_stan - ile_odjac
                    if nowy_stan < 0:
                        st.error(f"Zbyt mała ilość! (Dostępne: {aktualny_stan})")
                    else:
                        supabase.table("Produkt").update({"liczba": nowy_stan}).eq("id", prod_id).execute()
                        st.success(f"Zaktualizowano stan. Pozostało: {nowy_stan}")
                        st.rerun()

        with col2:
            st.subheader("🗑️ Usuwanie całkowite")
            with st.form("del_form"):
                prod_to_del = st.selectbox("Produkt do usunięcia", options=df_p['nazwa'].tolist())
                if st.form_submit_button("🧨 Usuń trwale", type="primary"):
                    p_id = int(df_p[df_p['nazwa'] == prod_to_del]['id'].values[0])
